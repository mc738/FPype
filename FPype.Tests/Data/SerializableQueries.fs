...
type SerializableQueryTests() =
    ...
    [<TestMethod>]
    member _.``Convert like condition to and from json``() =
        let condition =
            SerializableQueries.Condition.Like(
                SerializableQueries.Value.Field
                    { TableName = "test_table"
                      Field = "foo" },
                SerializableQueries.Value.Boolean true
    ...
    [<TestMethod>]
    member _.``Convert and condition to and from json``() =
        let condition =
            SerializableQueries.Condition.And(
                SerializableQueries.Condition.Equals(
                    SerializableQueries.Value.Field
                        { TableName = "test_table"
                          Field = "foo" },
    ...
    member _.``Convert not condition to and from json``() =
        let condition =
            SerializableQueries.Condition.Not(
                SerializableQueries.Condition.Equals(
                    SerializableQueries.Value.Field
                        { TableName = "test_table"
                          Field = "foo" },
    ...
    [<TestMethod>]
    member _.``Convert join (inner) to and from json``() =
        let join =
            ({ Type = SerializableQueries.JoinType.Inner
    ...
    [<TestMethod>]
    member _.``Convert join (outer) to and from json``() =
        let join =
            ({ Type = SerializableQueries.JoinType.Outer
               Table = { Name = "table_2"; Alias = None }
               Condition =
                 SerializableQueries.Condition.Equals(
                     SerializableQueries.Value.Field
