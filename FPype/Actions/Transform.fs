...
module Transform =
        ...
        let run (parameters: Parameters) (stepName: string) (store: PipelineStore) =
            ...
            |> Result.map (fun r ->
                store.Log(
                    stepName,
                    name,
                    $"Executed query and saved {r.Length} row(s) to table `{parameters.Table.Name}`."
                )
        ...
        let run (parameters: Parameters) (stepName: string) (store: PipelineStore) =
            ...
            |> Result.map (fun r ->
                store.Log(
                    stepName,
                    name,
                    $"Aggregated and saved {r.Length} row(s) to table `{parameters.Table.Name}`."
                )
    ...
    module ``aggregate-by-date`` =
        ...
        let run (parameters: Parameters) (stepName: string) (store: PipelineStore) =
            ...
            // TODO need to test this change! From List.map to List.fold
            parameters.DateGroups.Groups
            |> List.fold
                (fun (result: Result<PipelineStore, string>) dg ->
                    match result with
                    | Ok store ->
                        ...
                        let r =
                            store.BespokeSelectRows(
                                parameters.Table,
                                $"{parameters.SelectSql} {condition}",
                                queryParameters
                            )
                        ...
                        |> Result.map (fun r ->
                            store.Log(
                                stepName,
                                name,
                                $"Aggregated and saved {r.Length} row(s) to table `{parameters.Table.Name}` for date group {dg.StartDate} - {dg.EndDate}."
                            )
    ...
    module ``map-to-object`` =
        ...
        let run (mapper: TableObjectMap) (stepName: string) (store: PipelineStore) =
                ...
                let rows =
                    ...
                    |> List.fold
                        (fun (acc) r ->
                            ...
                            | Ok row -> row :: acc
                            | Error e ->
                                store.LogError(stepName, name, $"Error creating row: {e}")
                                acc)
                        ([])
                    |> List.rev
    ...
    module ``merge-results`` =
        ...
        let run (parameters: Parameters) (stepName: string) (store: PipelineStore) =
            ...
            |> store.InsertRows
            |> Result.map (fun r ->
                store.Log(stepName, name, $"Merged and saved {r.Length} row(s) to table `{parameters.Table.Name}`.")
                store)
    ...
    module ``pivot`` =
        ...
        let run () (store: PipelineStore) =
            ...
            let q2 =
                "SELECT avg_variance, entry_year, entry_month, entry_day FROM total GROUP BY GROUP BY entry_year, entry_month, entry_day"

            let q3 =
                "SELECT avg_variance FROM by_cat WHERE industry = @0 AND entry_year = @1 AND entry_month = @2 AND entry_day = @3;"
