...
module Common =
    ...
    type TableResolver = { GetName: unit -> string }
    ...
    type PipelineAction =
        ...
        static member Create(name, stepName, action) =
            { Name = name
              StepName = stepName
              Action = action }
                    ...
                    | Some a ->
                        ...
                        | false ->
                            try
                                ...
                                fs.Flush() |> Ok
                            with exn ->
                                Error $"Error creating file `{path}`: {exn.Message}"
    ...
    let toJsonElement (str: string) =
        ...
        with exn ->
            Error $"Failed to deserialize json element. Error: {exn.Message}"
